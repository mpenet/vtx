...
(fn render-status [offset height dn search-idx search-matches show-line-nums wrap]
  (let [last (math.min (+ offset height) dn)
        ...
        wrap-hint (if wrap
                      " [wrap]"
                      "")]
    (.. "\r" (ansi.style (.. "lines " (+ offset 1) "-" last "/" dn " " pct match-info " - q:quit /:search g/G l:ln# w:wrap" ln-hint wrap-hint) ansi.reverse) ansi.screen.clear-right)))
...
"))))
                           (if search-mode
                               ...
                               (term.write (render-status offset height dn search-idx search-matches show-line-nums wrap)))
                           (if search-mode
                           ...
                           (let [k (term.read-key)]
                             (if search-mode
                                 ...
                                 (match k
                                   ...
                                   (where (or " " "page-down")) (set offset (math.min max-off (+ offset height)))
                                   "/" (do
                                   ...
                                   (where (or "q" "\003")) (set running false))))))) {:alt-screen opts.alt-screen})
