...
(local autocomplete-m (require "irx.widget.autocomplete"))

(local date-picker-m (require "irx.widget.date-picker"))

(local checklist-m (require "irx.widget.checklist"))

(local choose-m (require "irx.widget.choose"))

(local confirm-m (require "irx.widget.confirm"))

(local file-picker-m (require "irx.widget.file-picker"))

(local filter-m (require "irx.widget.filter"))

(local input-m (require "irx.widget.input"))

(local num-input-m (require "irx.widget.num-input"))

(local password-m (require "irx.widget.password"))

(local radio-m (require "irx.widget.radio"))
...
(local write-m (require "irx.widget.write"))
        ...
        (while (not field-done)
          (let [field-opts (or field.opts {})
                ...
                val (match field.type
                      ...
                      _ (error (.. "irx.form: unknown field type " (tostring field.type))))]
            (if (= val nil)
                (do
                  (set aborted true)
                  (set field-done true))
