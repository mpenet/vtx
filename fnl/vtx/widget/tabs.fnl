...
(local default-opts {:active-fg ansi.fg.cyan :inactive-fg ansi.dim})

(fn render-tab-bar [tab-list active-i opts ?term-w]
  (let [term-w (or ?term-w 80)
        tabs (icollect [i tab (ipairs tab-list)] {:active (= i active-i) :inner-w (+ (# tab.label) 2) :label tab.label})
        n (# tabs)
        total-w (+ (accumulate [s 0 _ t (ipairs tabs)] (+ s t.inner-w 2)) (math.max 0 (- n 1)))
        trailing-w (math.max 0 (- term-w total-w))
        row0 (table.concat (icollect [i t (ipairs tabs)] (.. (ansi.style (.. "╭" (string.rep "─" t.inner-w) "╮") opts.active-fg) (if (< i n)
                                                                                                                                           " "
                                                                                                                                           ""))))
        row1 (table.concat (icollect [i t (ipairs tabs)] (.. (if t.active
                                                                 (ansi.style (.. "│ " t.label " │") ansi.bold opts.active-fg)
                                                                 (.. (ansi.style "│" opts.active-fg) (ansi.style (.. " " t.label " ") opts.inactive-fg) (ansi.style "│" opts.active-fg))) (if (< i n)
                                                                                                                                                                                                  " "
                                                                                                                                                                                                  ""))))
        row2 (ansi.style (.. (table.concat (icollect [i t (ipairs tabs)] (.. (if t.active
                                                                                 (.. "╯" (string.rep " " t.inner-w) "╰")
                                                                                 (.. "┴" (string.rep "─" t.inner-w) "┴")) (if (< i n)
                                                                                                                                    "─"
                                                                                                                                    "")))) (string.rep "─" trailing-w)) opts.active-fg)]
    (.. row0 "\n" row1 "\n" row2)))
