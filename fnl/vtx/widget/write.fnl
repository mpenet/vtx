...
"))))
                         (term.write (.. "\r" (ansi.style (.. "C-c C-c: submit · C-q: quit · C-z: undo · " (# lines) " line(s)") ansi.dim) ansi.screen.clear-right))
                       ...
                       (let [k (term.read-key)]
                         (match k
                           {:paste content} (let [paste-lines (util.split-lines content)
                                                  np (# paste-lines)
                                                  cur-line (. lines row)
                         ...
                         (when opts.on-change
                           (let [content (table.concat lines "\n")]
                             (when (not= content prev-content)
                               ...
                               (set prev-content content))))))) {:bracketed-paste true})
    (term.cursor-up top-distance)
    (let [h (widget-height)]
      (term.clear-rows h))
    result))
