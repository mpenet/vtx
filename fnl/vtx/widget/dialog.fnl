...
(fn render-buttons [buttons active opts]
  (let [parts (icollect [i label (ipairs buttons)] (if (= i active)
      ...
      (term.with-raw (fn []
                       ...
                       (while running
                         (let [content (.. message nl nl (render-buttons buttons active opts))
                               box (style-m.style content {:border opts.border :fg opts.fg :padding opts.padding :width opts.width})
