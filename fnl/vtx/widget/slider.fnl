    ...
    (term.with-raw (fn []
                     ...
                     (while running
                       (let [bar (render-slider value opts)
                             label (if opts.format-fn
                                       (opts.format-fn value)
                                       (tostring value))]
                         (term.write (.. "\r" opts.prompt bar " " label ansi.screen.clear-right))
                         (let [k (term.read-key)]
                           (match k
                             (where (or "left" "h" "\002")) (set value (math.max opts.min (- value opts.step)))
                             (where (or "right" "l" "\006")) (set value (math.min opts.max (+ value opts.step)))
