    ...
    (if (and (= (# buf) 0) (> (# opts.placeholder) 0))
        ...
        (let [before (buf:sub 1 pos)
              ...
              after (if (> (# buf) (+ pos 1))
                        (buf:sub (+ pos 2))
                        "")
              ghost-str (if (and ghost (> (# ghost) 0))
                            (ansi.style ghost ansi.dim)
                            "")]
          (.. "\r" prompt before cur after ghost-str ansi.screen.clear-right)))))
...
(fn input [user-opts]
  (let [opts (collect [k v (pairs default-opts)] k v)]
    ...
    (var saved-buf "")
    (fn update-ghost []
      (set ghost (if (and opts.complete (> (# buf) 0))
                     (let [cands (opts.complete buf)]
                       (if (and cands (> (# cands) 0))
                           (let [first (. cands 1)]
                             (if (and (> (# first) (# buf)) (= (: (first:sub 1 (# buf)) "lower") (buf:lower)))
                                 (first:sub (+ (# buf) 1))
                                 ""))
                           ""))
                     "")))
