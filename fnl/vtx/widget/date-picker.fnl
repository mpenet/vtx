...
(local default-opts {:active-fg ansi.fg.cyan :fg ansi.fg.white :prompt "Date: " :separator "-"})
      ...
      (term.with-raw (fn []
                       ...
                       (while running
                         (let [sep opts.separator
                               ...
                               row (.. (ansi.style opts.prompt opts.fg) (if (= seg 1)
                                                                            (ansi.style ys ansi.bold opts.active-fg)
                                                                            (ansi.style ys opts.fg)) sep (if (= seg 2)
                                                                                                             (ansi.style ms ansi.bold opts.active-fg)
                                                                                                             (ansi.style ms opts.fg)) sep (if (= seg 3)
                                                                                                                                              (ansi.style ds ansi.bold opts.active-fg)
                                                                                                                                              (ansi.style ds opts.fg)))]
                           (term.write (.. "\r" row ansi.screen.clear-right))
                           (let [k (term.read-key)]
                             (match k
                               (where (or "\t" "right" "l")) (set seg (if (= seg 3)
                                                                          1
                                                                          (+ seg 1)))
                               (where (or "left" "h")) (set seg (if (= seg 1)
                                                                    3
                                                                    (- seg 1)))
                               (where (or "up" "\016")) (match seg
                                                          1 (set year (+ year 1))
                                                          2 (do
                                                              (set month (if (= month 12)
                                                                             1
                                                                             (+ month 1)))
                                                              (set day (clamp-day year month day)))
                                                          3 (set day (math.min (days-in-month year month) (+ day 1))))
                               (where (or "down" "\014")) (match seg
                                                            1 (set year (math.max 1 (- year 1)))
                                                            2 (do
                                                                (set month (if (= month 1)
                                                                               12
                                                                               (- month 1)))
                                                                (set day (clamp-day year month day)))
                                                            3 (set day (math.max 1 (- day 1))))
                               (where (or "\r" "\n")) (do
                                                        (set result (string.format "%04d-%02d-%02d" year month day))
                                                        (set running false))
